...
export async function POST(req: Request) {
  try {
    ...
    if (!skin || typeof skin !== 'string') {
      return NextResponse.json({ error: 'Скин не предоставлен или имеет неверный формат' }, { status: 400 });
    }
    ...
    return NextResponse.json({ message: 'Скин успешно загружен!' }, { status: 200 });
  } catch (error) {
